# PILOT PHASE: Comparative Learning Experiment

## AI-Mediated vs. Autodidactic Study in Philosophy of Mind

**A Seven-Week Investigation into Learning Methods**

---

**Document Version:** 1.1 (Corrected)  
**Created:** November 2025  
**Purpose:** Complete working curriculum for experimental pilot phase

---

## Table of Contents

1. [Overview and Rationale](#overview-and-rationale)
2. [Pre-Pilot Preparation (3-4 days)](#pre-pilot-preparation)
3. [Pilot A: Autodidactic Condition (Weeks 1-3)](#pilot-a-autodidactic-condition)
4. [Transition Week](#transition-week)
5. [Pilot B: AI-Mediated Condition (Weeks 4-6)](#pilot-b-ai-mediated-condition)
6. [Week 7: Comparative Analysis and Decision](#week-7-comparative-analysis)
7. [Complete Reading List with Citations](#complete-reading-list)
8. [Templates and Forms](#templates-and-forms)
9. [Five-Dimension Assessment Rubric](#assessment-rubric)

---

## Overview and Rationale

### The Central Question

**Does AI guidance produce better learning outcomes than pure autodidacticism?**

Before committing to a full four-year programme of AI-mediated study in cognitive-cultural science, this pilot tests whether the elaborate AI framework adds sufficient value to justify its complexity.

### The Experimental Design

The pilot compares two parallel learning experiences:

- **Pilot A (Weeks 1-3):** Study philosophy of mind (consciousness and intentionality) entirely through self-directed reading, with **no AI assistance**
- **Pilot B (Weeks 4-6):** Study philosophy of mind (mind-body problem and embodiment) with **continuous AI guidance and feedback**
- **Week 7:** Systematic comparison and evidence-based decision about proceeding

### What Makes This Rigorous

This is not casual self-improvement but a methodologically structured investigation:

- **Controlled comparison:** Similar topics, similar difficulty, similar time investment
- **Multiple data sources:** Essays, daily logs, self-assessments, ethnographic reflections
- **Genuine openness to negative results:** If AI adds little value, that's a legitimate and important finding
- **Clear decision criteria:** Week 7 requires justified decision based on evidence, not assumptions
- **Failure is productive:** The pilot succeeds if it reveals what works and what doesn't

### Success Criteria

**The pilot succeeds if:**

- [ ] You complete all reading and writing requirements for both conditions
- [ ] You maintain consistent documentation (daily logs, weekly reflections)
- [ ] Your Week 7 analysis demonstrates genuine reflection, not going through motions
- [ ] You make an evidence-based decision about proceeding
- [ ] You learn something valuable regardless of which condition "wins"

**The pilot fails if:**

- [ ] You skip readings or rush through without real engagement
- [ ] Documentation becomes perfunctory ("I read today. It was fine.")
- [ ] Week 7 analysis is superficial or predetermined
- [ ] Decision is based on sunk costs rather than actual experience
- [ ] You treat the pilot as a formality rather than genuine experiment

### Time Commitment

- **Weeks 1-3 (Pilot A):** 12-15 hours per week
- **Transition Week:** 2-3 days off
- **Weeks 4-6 (Pilot B):** 12-15 hours per week  
- **Week 7:** 10-12 hours for analysis

**Total:** Approximately 90-100 hours over 7 weeks

---

## Pre-Pilot Preparation

**Duration:** 3-4 days before Week 1 begins

### Task 1: Environment Setup

Create this directory structure on your computer:

```
CognitiveCulturalScience/
├── Pilot/
│   ├── PilotA_Autodidactic/
│   │   ├── readings/
│   │   ├── notes/
│   │   ├── essay/
│   │   └── daily_log.md
│   ├── PilotB_AIMediated/
│   │   ├── readings/
│   │   ├── notes/
│   │   ├── essay/
│   │   ├── ai_interactions/
│   │   └── ethnographic_reflections.md
│   └── PilotComparison/
│       └── final_analysis.md
└── Module1/ (for later, if proceeding)
```

---

### Task 2: Acquire All Readings

You need to obtain these texts before starting. Allow 2-3 days for library access or purchase.

#### For Pilot A (Weeks 1-3): Consciousness

1. **Bermúdez, J.L.** (2020). *Philosophy of Psychology: A Contemporary Introduction* (2nd ed.). Routledge.
   - **You need:** Chapter 5 (Consciousness and Subjectivity), approximately 40 pages

2. **Nagel, T.** (1974). "What Is It Like to Be a Bat?" *The Philosophical Review*, 83(4), 435-450.
   - **Where to find:** JSTOR, or in Nagel's book *Mortal Questions* (1979)
   - **Length:** 8 pages

3. **Dennett, D.** (1991). *Consciousness Explained*. Little, Brown and Company.
   - **You need:** Chapter 2 (Explaining Consciousness), approximately 30 pages

#### For Pilot B (Weeks 4-6): Mind-Body Problem and Embodiment

1. **Bermúdez, J.L.** (2020). *Philosophy of Psychology* (2nd ed.).
   - **You need:** Chapter 8 (The Mind-Body Problem), approximately 35 pages

2. **Merleau-Ponty, M.** (1945/2012). *Phenomenology of Perception* (D. Landes, Trans.). Routledge.
   - **You need:** Preface and Introduction only, approximately 30 pages
   - **Note:** This is challenging—don't try to read the whole book

3. **Churchland, P.S.** (1986). *Neurophilosophy: Toward a Unified Science of the Mind-Brain*. MIT Press.
   - **You need:** Part II: "Recent Developments in the Philosophy of Science" (approximately 100-150 pages)
   - **Note on structure:** The book has three parts:
     - Part I: "Some Elementary Neuroscience" (skip unless you need neuroscience context)
     - Part II: "Recent Developments in the Philosophy of Science" (READ THIS—contains her philosophical arguments)
     - Part III: "A Neurophilosophical Perspective" (dated 1980s computational models—skip)
   - **Where to find:** Through Open University library (check JSTOR, ProQuest, or direct MIT Press institutional access at https://direct.mit.edu/books/monograph/4792). ISBN: 9780262530859 (paperback) or 0262031167 (hardcover)

4. **Ingold, T.** (2000). *The Perception of the Environment: Essays on Livelihood, Dwelling and Skill*. Routledge.
   - **You need:** 
     - Chapter 1: The Organism and its Environment (pp. 13-25)
     - Chapter 16: On Weaving a Basket (pp. 339-348)
   - **Total:** approximately 20 pages

5. **Mumford, L.** (1964). "Authoritarian and Democratic Technics." *Technology and Culture*, 5(1), 1-8.
   - **Where to find:** JSTOR or Project MUSE
   - **Length:** 8 pages

#### Optional (for deeper exploration):

- Chalmers, D. (1995). "Facing Up to the Problem of Consciousness"
- Stanford Encyclopedia of Philosophy entries on consciousness, dualism, physicalism

---

### Task 3: Baseline Self-Assessment

**Due:** Before Week 1 begins  
**Length:** 500 words

Write a document titled **"Pre-Pilot Baseline"** answering these questions:

**1. What is my current understanding of the mind-body problem?**
- Don't research—write what you currently think
- What is consciousness?
- How does mind relate to brain?

**2. What positions do I currently hold?**
- Am I a dualist (mind and body are separate)?
- Am I a physicalist (mind is just brain)?
- Am I unsure?
- Why do I hold this position?

**3. How confident am I in these beliefs?**
- Rate yourself 1-10
- Explain the rating

**4. What do I hope to learn from this pilot?**
- About the topic (consciousness, mind-body problem)
- About learning methods (autodidactic vs. AI-mediated)
- About my own learning capabilities

**5. What would constitute "success" in each condition?**
- What would make Pilot A successful?
- What would make Pilot B successful?
- What would make the overall pilot successful?

**Save this document.** You'll return to it in Week 7 to measure how much your understanding changed.

---

### Task 4: Set Up Templates

Copy the templates from the [Templates section](#templates-and-forms) into your directory structure:

- `daily_log_template.md` → copy to `/PilotA_Autodidactic/`
- `weekly_reflection_template.md` → copy to `/PilotB_AIMediated/`
- `essay_self_assessment_template.md` → copy to both pilot directories

---

### Task 5: Schedule Commitment

**Block time in your calendar now:**

- **Pilot A:** 2-2.5 hours per day, 6 days per week, for 3 weeks
- **Transition:** 2-3 days completely off
- **Pilot B:** 2-2.5 hours per day, 6 days per week, for 3 weeks
- **Week 7:** 10-12 hours across the week

**Choose your start date:** Pick a Monday to begin Week 1 (Pilot A)

**Mark it in your calendar and commit.**

---

## Pilot A: Autodidactic Condition

**Duration:** Weeks 1-3  
**Topic:** Consciousness and Intentionality  
**Central Question:** Can third-person science capture first-person experience?

### The Learning Constraint

**YOU MAY:**
- Read the assigned texts at your own pace
- Take notes in your own words
- Consult dictionaries and encyclopedias
- Use Stanford Encyclopedia of Philosophy for definitions
- Reread difficult passages
- Look up unfamiliar philosophical terms
- Use your existing knowledge and reasoning

**YOU MAY NOT:**
- Ask Claude (or any AI) questions about the readings
- Use AI to help understand, interpret, or discuss the material
- Seek secondary sources that interpret these texts (no summaries, lectures, explainer videos)
- Discuss readings with other humans (to maintain comparison with Pilot B's isolation)

**The principle:** You are entirely dependent on primary texts and your own intellectual resources.

---

### Week 1: Reading and Understanding

**Goal:** Develop initial understanding of consciousness as a philosophical problem

**Reading Assignment:**
1. Bermúdez, Chapter 5: Consciousness and Subjectivity (~40 pages)
2. Nagel, "What Is It Like to Be a Bat?" (~8 pages)

#### Day 1: Monday (2-2.5 hours)

**Reading:**
- Bermúdez Chapter 5, first half (pages 1-20, to mid-chapter break)

**Focus on:**
- What is phenomenal consciousness?
- What is access consciousness?
- What is the "hard problem"?

**Tasks:**
- Take detailed notes on main concepts
- Write down terms you don't understand
- Look up unfamiliar terms in Stanford Encyclopedia
- Identify: What is Bermúdez's main question?

**Documentation:**
- Complete daily log entry
- Note: What was most difficult? What remains unclear?

---

#### Day 2: Tuesday (2-2.5 hours)

**Reading:**
- Bermúdez Chapter 5, second half (pages 21-40)

**Focus on:**
- How do different theories attempt to explain consciousness?

**Tasks:**
- Continue detailed notes
- Synthesize: What is the overall argument of this chapter?
- Create a concept map if helpful
- Reread confusing sections

**Documentation:**
- Daily log entry
- Note: How confident am I? (1-10)

---

#### Day 3: Wednesday (2-2.5 hours)

**Reading:**
- Nagel, "What Is It Like to Be a Bat?" (first read)

**Tasks:**
- Read slowly and carefully
- Take initial notes
- Identify: What is Nagel's central argument?
- What examples does he use?
- What is his conclusion about reduction?

**Documentation:**
- Daily log entry
- Note: What questions arise?

---

#### Day 4: Thursday (2-2.5 hours)

**Reading:**
- Nagel, "What Is It Like to Be a Bat?" (second read)

**Tasks:**
- Read more carefully, focus on argument structure
- More detailed notes
- Identify: What premises lead to Nagel's conclusion?
- Where are you most/least convinced?

**Documentation:**
- Daily log entry
- Write: What is the strongest part of Nagel's argument? The weakest?

---

#### Day 5: Friday (2-2.5 hours)

**Review and Synthesis:**
- Review all notes from Bermúdez and Nagel
- Identify connections between them

**Writing Task:**
- Write 500-word summary of Nagel's argument
- Purpose: Can you explain his position clearly to yourself?

**Documentation:**
- Daily log entry
- Note: What remains unclear?

---

#### Day 6: Saturday (1-1.5 hours)

**Rest and Reflection:**
- Light review, or take the day off
- Let the week's reading settle

**Documentation:**
- Brief daily log entry

---

#### Week 1 Reflection (Due Sunday)

**Length:** 500 words

**Address:**
1. What did I understand well this week?
2. What remains confusing?
3. How did I handle difficulties?
4. What strategies did I develop?
5. What do I wish I had access to?
6. Confidence assessment: How confident am I? (1-10) Why?

---

### Week 2: Deepening and Contrasting

**Goal:** Understand Dennett's response to Nagel and evaluate the debate

**Reading Assignment:**
1. Dennett, *Consciousness Explained* Chapter 2 (~30 pages)
2. Return to Nagel with Dennett in mind

#### Day 1: Monday (2.5-3 hours)

**Reading:**
- Dennett Chapter 2, first half (pages 1-15)
- **Note:** Dennett is dense—may take longer

**Tasks:**
- Take careful notes
- Identify: What is Dennett trying to accomplish?
- What is his method (heterophenomenology)?
- How does his approach differ from Nagel's?

**Documentation:**
- Daily log entry
- Note: How am I handling this difficulty?

---

#### Day 2: Tuesday (2.5-3 hours)

**Reading:**
- Dennett Chapter 2, second half (pages 16-30)

**Focus on:**
- Dennett's positive account of consciousness

**Tasks:**
- Complete notes on Dennett
- Synthesize: What is Dennett's main claim?
- What is his critique of Nagel?

**Documentation:**
- Daily log entry

---

#### Day 3: Wednesday (2 hours)

**Review and Compare:**
- Reread key sections of Dennett
- No new reading

**Tasks:**
- Identify: What does Dennett think is wrong with Nagel?
- Where do they disagree most fundamentally?

**Documentation:**
- Daily log entry
- Write: What is the crux of their disagreement?

---

#### Day 4: Thursday (2 hours)

**Comparative Analysis:**

**Task:** Create detailed comparison table:

| Aspect | Nagel | Dennett |
|--------|-------|---------|
| What is consciousness? | [Your summary] | [Your summary] |
| Can it be studied scientifically? | | |
| What is subjective experience? | | |
| Can third-person methods capture it? | | |
| Main argument | | |
| Key examples/evidence | | |
| Strengths | | |
| Weaknesses | | |

**Documentation:**
- Daily log entry
- Save this table for essay

---

#### Day 5: Friday (2 hours)

**Critical Evaluation:**
- Return to Nagel with Dennett's critique in mind
- Reread "What Is It Like to Be a Bat?" (third time, quick)

**Tasks:**
- Who is more persuasive? Why?
- What would Nagel say to Dennett?
- What would Dennett say back?
- Where do you stand?

**Documentation:**
- Daily log entry
- Note: Beginning to formulate essay argument

---

#### Day 6: Saturday (1.5 hours)

**Essay Planning:**
- Begin outlining essay (detailed outline, not writing yet)

**Essay Question:**  
**"Nagel vs. Dennett: Can Third-Person Science Capture First-Person Experience?"**

**Outline should include:**
- Your thesis
- Main sections with key points
- Evidence from texts

**Documentation:**
- Daily log entry
- Save outline

---

#### Week 2 Reflection (Due Sunday)

**Length:** 500 words

**Address:**
1. How has my understanding developed?
2. Where am I still stuck?
3. How confident am I in interpreting these thinkers? (1-10 for each)
4. What strategies have I developed for difficult texts?
5. How does learning without guidance feel?

---

### Week 3: Writing and Self-Assessment

**Goal:** Produce essay demonstrating understanding; rigorously assess your own work

**No new reading—consolidation and writing week**

#### Day 1: Monday (3 hours)

**Writing:**
- Begin drafting essay
- Target: Complete Introduction + Section 1 (Nagel's position)
- Aim for ~600 words

**Essay Structure (1,500 words total):**

**I. Introduction (200 words)**
- Frame the debate: What's at stake?
- Brief overview of positions
- Your thesis

**II. Nagel's Position (400 words)**
- His argument from "What Is It Like to Be a Bat?"
- Key examples
- Why consciousness resists reduction
- Concept of "subjective character of experience"

**III. Dennett's Response (400 words)**
- His critique of Nagel
- What is heterophenomenology?
- His alternative account
- Why consciousness can be explained scientifically

**IV. Critical Analysis (400 words)**
- Strengths and weaknesses of each
- Your evaluation: Who is more persuasive?
- What philosophical assumptions underlie disagreement?

**V. Conclusion (100 words)**
- Summarize your position
- Implications for cognitive science

**Documentation:**
- Daily log entry
- Save draft

---

#### Day 2: Tuesday (3 hours)

**Writing:**
- Continue drafting
- Complete Sections II and III (Nagel and Dennett)
- Aim for ~1,000 words total

**Quality Check:**
- Am I accurately representing both thinkers?
- Am I using evidence from texts?
- Is my writing clear?

**Documentation:**
- Daily log entry

---

#### Day 3: Wednesday (3 hours)

**Writing:**
- Complete Section IV (Critical Analysis)
- Write Conclusion
- Complete first full draft (~1,500 words)

**Self-Check:**
- Have I answered the essay question?
- Is my argument clear?
- Have I fairly represented both positions?

**Documentation:**
- Daily log entry
- Note: How do I feel about this draft?

---

#### Day 4: Thursday (2.5 hours)

**Revision:**
- Reread entire draft with fresh eyes

**Check for:**
- **Clarity:** Is each paragraph's point clear?
- **Evidence:** Have I supported claims with textual evidence?
- **Accuracy:** Have I represented Nagel and Dennett correctly?
- **Argument:** Does my analysis build logically?
- **Style:** Is prose clear and precise?

**Revise accordingly**

**Documentation:**
- Daily log entry
- Note specific improvements made

---

#### Day 5: Friday (2 hours)

**Final Revision and Proofreading:**
- Make final improvements
- Proofread carefully
- Ensure citations are correct

**Finalize essay**

**Documentation:**
- Daily log entry

---

#### Day 6: Saturday (2-3 hours)

**SELF-ASSESSMENT (800 words)**

This is crucial—assess your own work rigorously using the five-dimension rubric.

**For each dimension, write:**

**1. Argument & Structure** (First/2:1/2:2/Third/Fail)
- What grade would I assign?
- Why? (Strengths and weaknesses)
- Specific evidence from essay

**2. Knowledge & Understanding**
- Assessment and justification

**3. Integration of Perspectives**
- Assessment and justification

**4. Critical Engagement**
- Assessment and justification

**5. Style, Evidence & Presentation**
- Assessment and justification

**Overall Indicative Grade:**
- First (70+), 2:1 (60-69), 2:2 (50-59), Third (40-49), Fail (<40)
- Overall justification: Why this grade?
- What would move it higher?

**Save with essay**

**Documentation:**
- Daily log entry

---

#### Week 3 Final Reflection (Due Sunday)

**Length:** 800 words  
**Title:** "Pilot A Complete: What I Learned"

**Required elements:**
1. What did I learn about consciousness, Nagel, and Dennett?
2. How confident am I in that learning? (1-10, with explanation)
3. What was hardest about learning without guidance?
4. What strategies did I develop?
5. What would have helped that I didn't have?
6. How do I feel about the essay I produced?
7. What are the limitations of what I learned?

---

**END OF PILOT A**

**Materials to Save:**
- Essay (final version)
- Self-assessment (800 words)
- All daily logs (18 entries)
- Three weekly reflections (500 words each)
- Week 3 final reflection (800 words)

**DO NOT review these until Week 7**

**DO NOT discuss with AI until Week 7**

---

## Transition Week

**Duration:** 2-3 days between pilots

### Purpose

Rest and approach Pilot B fresh, without fatigue.

### Rules

**DO:**
- Take genuine time off from academic work
- Engage in different activities
- Rest mentally and physically
- Organize Pilot B materials

**DON'T:**
- Think about consciousness or mind-body problems
- Reread Pilot A materials
- Begin Pilot B readings early
- Compare the conditions yet

### Optional: Embodied Learning Micro-Pilot

**Duration:** 2-3 hours over 2 days (optional)

**Activity:** Learn something through practice, not reading

**Options:**
- Work with a real dataset (statistical analysis)
- Conduct and transcribe an interview
- Learn a memory technique and practice it
- Try meditation and document phenomenology

**Documentation:**
- Brief essay (500 words): "What I Learned By Doing Rather Than Reading"
- Question: How does embodied engagement differ from textual learning?

**Purpose:** Introduces embodied/practical knowledge as comparison point. Connects to Ingold's theory in Pilot B.

**This is entirely optional.**

---

## Pilot B: AI-Mediated Condition

**Duration:** Weeks 4-6  
**Topic:** Mind-Body Problem and Embodiment  
**Central Questions:** What is the relationship between mind and body? Can consciousness be reduced to brain processes? What does embodiment reveal that reduction obscures?

### The Learning Enhancement

**YOU MAY:**
- Ask Claude questions at any point
- Request clarification, interpretation, guidance
- Submit reading notes for feedback
- Discuss confusions as they arise
- Ask for help structuring thinking
- Request feedback on essay drafts
- Use Claude as intellectual interlocutor

**YOU MAY NOT:**
- Use other AI systems (maintain consistency)
- Seek human guidance (maintain comparison)
- Have Claude write your essay (you must write; Claude provides feedback only)

**The principle:** You have access to AI expertise, but intellectual work remains yours.

---

### Week 4: Reading with AI Guidance

**Goal:** Understand positions on mind-body problem, with emphasis on phenomenology

**Reading Assignment:**
1. Bermúdez, Chapter 8: The Mind-Body Problem (~35 pages)
2. Merleau-Ponty, *Phenomenology of Perception* - Preface & Introduction (~30 pages)

#### Before You Begin: Initial AI Consultation

**On Sunday evening or Monday morning, ask Claude:**

"I'm beginning Pilot B of my comparative learning experiment. The topic is the mind-body problem, moving from historical dualism through reductionism to embodiment theory. I'll be reading:
- Bermúdez's overview (Chapter 8)
- Merleau-Ponty's phenomenology
- Churchland's neurophilosophy (next week)
- Ingold's embodied anthropology (next week)

Before I begin:
1. What should I prioritize in Bermúdez Chapter 8?
2. What key concepts should I watch for?
3. How does the progression from Descartes through behaviorism to functionalism set up contemporary debates?
4. What's likely to be most difficult?
5. What connections should I track?"

**Document response in:** `/ai_interactions/week4_initial_consultation.md`

---

#### Day 1: Monday (2-2.5 hours)

**Reading:**
- Bermúdez Chapter 8, pages 1-12 (Descartes, dualism, behaviorism)

**Tasks:**
- Read with attention to Claude's guidance
- Take notes
- **Ask Claude questions as they arise:**
  - Example: "Bermúdez discusses Descartes' interactionism. Why was the pineal gland important to his theory?"

**Evening Check-in:**
- **Ask Claude:** "Here are my notes [paste]. Have I understood correctly? What have I missed?"
- Adjust understanding based on feedback

**Documentation:**
- `/ai_interactions/week4_day1.md`
- Brief daily log: What questions? How helpful?

---

#### Day 2: Tuesday (2-2.5 hours)

**Reading:**
- Bermúdez Chapter 8, pages 13-24 (identity theory, functionalism)

**Tasks:**
- Continue reading and notes
- **Ask questions:**
  - Example: "Token vs. type identity theory—I don't understand the distinction. Can you clarify with examples?"

**Evening Check-in:**
- **Ask Claude:** "My understanding of functionalism is [explanation]. Is this accurate? What nuances am I missing?"

**Documentation:**
- `/ai_interactions/week4_day2.md`
- Daily log

---

#### Day 3: Wednesday (2-2.5 hours)

**Reading:**
- Bermúdez Chapter 8, pages 25-35 (physicalism, contemporary debates)

**Tasks:**
- Complete chapter
- **Ask Claude:** "I've finished Bermúdez Chapter 8. Help me identify the main argument arc. Here's what I think: [summary]"
- Refine understanding

**Evening Check-in:**
- **Ask Claude:** "Before Merleau-Ponty tomorrow, help me understand: What are the main positions on mind-body problem? How do they relate historically and conceptually?"
- Construct conceptual map with Claude

**Documentation:**
- `/ai_interactions/week4_day3.md`
- Daily log

---

#### Day 4: Thursday (2.5-3 hours)

**Reading:**
- Merleau-Ponty, Preface (first half)

**Before Reading:**
- **Ask Claude:** "I'm about to read Merleau-Ponty's Preface. How does his phenomenology relate to positions Bermúdez outlined? What should I look for? What will be difficult?"

**Tasks:**
- Read with Claude's framing
- Take notes
- **Ask frequently (this is challenging):**
  - Example: "Merleau-Ponty talks about 'lived body' (corps vécu). What does he mean? How is it different from body-as-object?"

**Evening Check-in:**
- **Ask Claude:** "Here's my understanding of the Preface [summary]. Am I on track?"

**Documentation:**
- `/ai_interactions/week4_day4.md`
- Daily log

---

#### Day 5: Friday (2.5-3 hours)

**Reading:**
- Merleau-Ponty, Preface (second half) and Introduction

**Tasks:**
- Complete Merleau-Ponty
- **Extended dialogue:**
  - "Let's discuss Merleau-Ponty's critique of dualism. He rejects both Cartesian dualism and reductive physicalism. What's his alternative? How does 'embodiment' differ from both?"

**Evening Check-in:**
- **Ask Claude:** "How does Merleau-Ponty challenge the positions Bermúdez outlined? What would he say to identity theorists or functionalists?"

**Documentation:**
- `/ai_interactions/week4_day5.md`
- Daily log: How has AI guidance changed my reading vs. Pilot A?

---

#### Day 6: Saturday (2 hours)

**Review and Consolidation:**
- Review week's readings with Claude

**Task:**
- **Ask Claude:** "Help me create a conceptual map of this week's learning. What are main positions? How do they relate? Where does Merleau-Ponty fit?"
- Construct clear, organized understanding

**Documentation:**
- Save conceptual map
- `/ai_interactions/week4_day6.md`
- Daily log

---

#### Week 4 Ethnographic Reflection (Due Sunday)

**Length:** 800 words  
**Title:** "Learning With AI: Week 4"

**Reflect ethnographically on AI relationship:**

1. **How did AI frame authority and expertise?**
   - Expert, collaborator, or tool?
   - How did language convey certainty?

2. **What questions did I ask? How helpful were responses?**
   - List 3-5 most important questions
   - Evaluate: Which most/least helpful?
   - Any confusing or unhelpful guidance?

3. **How did AI change my reading experience vs. Pilot A?**
   - Easier? More efficient? Deeper or shallower?
   - Did AI enable understanding I couldn't reach alone?
   - Or just confirm what I could figure out independently?

4. **Did AI create dependency or develop autonomy?**
   - Did I ask before trying to figure things out?
   - Or use AI strategically after attempting understanding?

5. **How did the AI relationship feel?**
   - Collaborative? Directive? Subordinate?
   - Did I trust responses? Why or why not?

6. **What would have been impossible without AI?**

7. **What did AI fail to provide?**

---

### Week 5: Deepening with AI as Interlocutor

**Goal:** Understand reductive neurophilosophy and its critique through embodiment

**Reading Assignment:**
1. Mumford, "Authoritarian and Democratic Technics" (~15 pages)
2. Churchland, *Neurophilosophy* Part II: "Recent Developments in the Philosophy of Science" (~100-150 pages)
3. Ingold, *The Perception of the Environment* - Selections (~20 pages)

**Note on Week 5 workload:** This is the most reading-intensive week of Pilot B due to Churchland's philosophical density. Plan accordingly—you may need 3-3.5 hours on Days 2-4.

#### Day 1: Monday (2 hours)

**Reading:**
- Mumford, "Authoritarian and Democratic Technics"

**Before Reading:**
- **Ask Claude:** "Give me context on Mumford's essay. What is his distinction between authoritarian and democratic technics? How might this relate to debates about consciousness and expertise?"

**Tasks:**
- Read with Claude's framing
- Focus on: What makes knowledge systems "authoritarian"?

**After Reading:**
- **Ask Claude:** "This week I read Churchland's neurophilosophy, which argues for reductive explanation. Help me think: Does Churchland's approach represent 'authoritarian' expertise—claiming to understand consciousness better than conscious subjects—or is it legitimate science? How should I think about this as I read her?"

**Documentation:**
- `/ai_interactions/week5_day1.md`
- Daily log

---

#### Day 2: Tuesday (3-3.5 hours)

**Reading:**
- Churchland, Part II: "Recent Developments in the Philosophy of Science" - First third (approximately pages 1-50 of Part II)

**Before Reading:**
- **Ask Claude:** "I'm beginning Churchland's Part II on philosophy of science and intertheoretic reduction. This is dense material. What's the overall structure of her argument? What should I prioritize? What will be most difficult?"

**Tasks:**
- Read carefully—this is dense philosophy of science
- Take detailed notes
- Focus on: What is her alternative to logical empiricist reduction?
- **Ask questions as needed:**
  - Example: "Churchland discusses 'intertheoretic reduction.' How does her account differ from Nagel-Schaffner reduction? Why does she need a post-empiricist theory?"

**Evening Check-in:**
- **Ask Claude:** "Here's what I understood from the first third of Part II [summary]. Is this accurate? What connections or distinctions am I missing? What should I watch for as I continue?"

**Documentation:**
- `/ai_interactions/week5_day2.md`
- Daily log: Note difficulty level and time spent

---

#### Day 3: Wednesday (3-3.5 hours)

**Reading:**
- Churchland, Part II - Second third (approximately pages 51-100)

**Tasks:**
- Continue through middle section of Part II
- Focus on: Her responses to dualist arguments and the multiple realizability objection
- **Ask Claude questions:**
  - Example: "Churchland responds to property dualism and qualia arguments. What's her strategy? How does this connect to her reduction account?"
  - Example: "She discusses the 'co-evolution' of neuroscience and psychology. What exactly does this methodology entail? How is it different from simply reducing psychology to neuroscience?"

**Evening Check-in:**
- **Ask Claude:** "I'm partway through Churchland's philosophical arguments. How do her responses to dualism and anti-reductionism set up her broader neurophilosophical project? What's the relationship between her theory of reduction and her eliminativist tendencies?"

**Documentation:**
- `/ai_interactions/week5_day3.md`
- Daily log

---

#### Day 4: Thursday (3-3.5 hours)

**Reading:**
- Churchland, Part II - Final third (approximately pages 101-150)
- Begin Ingold, Chapter 1: The Organism and its Environment (pp. 13-25)

**Tasks:**
- Complete Churchland Part II
- **Extended dialogue with Claude:**
  - "I've finished Churchland's Part II. Help me synthesize: What is the core of her reductive neurophilosophy? How does the 'co-evolution' methodology work? What does she think happens to folk psychology—revision or elimination? How does her position differ from eliminative materialism?"
  - Clarify and consolidate understanding before moving to Ingold

- Begin Ingold Chapter 1
- **Ask Claude:** "I'm moving from Churchland to Ingold. How does Ingold's ecological anthropology relate to philosophical debates about mind? What should I look for?"

**Evening Check-in:**
- **Ask Claude:** "Now that I've completed Churchland's philosophical framework and started Ingold's ecological anthropology, help me see: How do their approaches to mind/consciousness differ fundamentally? Both claim to be materialist and anti-Cartesian, but they seem to point in different directions. What would Ingold say about Churchland's reductionism?"

**Documentation:**
- `/ai_interactions/week5_day4.md`
- Daily log: Reflect on Churchland's difficulty and comprehension

---

#### Day 5: Friday (2.5 hours)

**Reading:**
- Ingold, Chapter 16: On Weaving a Basket (pp. 339-348)

**Tasks:**
- Complete Ingold readings
- Focus on: skilled practice, embodied knowledge, "enskillment"
- **Extended dialogue:**
  - "Let's discuss the relationship between Ingold and Churchland. Both claim to be materialist and anti-Cartesian, but approach consciousness differently. Help me think through:
    1. How would Ingold critique Churchland's reductionism?
    2. How would Churchland respond to Ingold's skilled practice and 'dwelling'?
    3. Are they addressing the same phenomenon or different aspects?
    4. Does Ingold's 'enskillment' escape reduction, or is it just complex neural processes Churchland would explain differently?"

**Documentation:**
- `/ai_interactions/week5_day5.md`
- Daily log: How is my AI use evolving? Better questions?

---

#### Day 6: Saturday (2 hours)

**Essay Planning with AI:**

**Essay Question:**  
**"Is Consciousness Reducible to Brain Processes? Evaluate with reference to Churchland and at least one critic."**

**Task:**
- **Ask Claude:** "I need to write this essay. Given my readings—Bermúdez, Merleau-Ponty, Churchland's Part II, Ingold, Mumford—help me think through:
  1. What should my thesis address? What are the key dimensions of the reduction question?
  2. How should I structure the essay?
  3. Which thinkers should I focus on? I could use Merleau-Ponty or Ingold as primary critic of Churchland—which offers stronger critical purchase?
  4. What connections or tensions should I highlight?
  5. How do I avoid simply summarizing positions—what level of critical analysis is needed?"

**Important:** Don't have Claude write outline—develop it yourself with guidance. Claude should help you think through structure, not provide the structure.

**Documentation:**
- Save your outline (your work, not Claude's)
- `/ai_interactions/week5_day6.md`
- Daily log

---

#### Week 5 Ethnographic Reflection (Due Sunday)

**Length:** 800 words  
**Title:** "Learning With AI: Week 5"

**Focus on evolution:**

1. **How has my AI use evolved from Week 4?**
   - Better questions?
   - More strategic?
   - Different kinds of questions?

2. **Have I become dependent or more autonomous?**
   - Evidence from this week
   - Did Churchland's difficulty make me more or less reliant on AI?

3. **Where did AI challenge vs. confirm my thinking?**
   - Specific examples
   - Did AI reveal blind spots or just validate existing understanding?

4. **What questions did AI answer well vs. poorly?**
   - Pattern recognition: What kinds of questions work best?
   - Where were responses inadequate or confusing?

5. **How does AI interlocutor compare to no interlocutor?**
   - Better? Worse? Different?
   - What would human interlocutor provide that AI doesn't?

6. **The Mumford question:**
   - Does AI feel like "authoritarian technics" (creating dependency)?
   - Or "democratic technics" (enhancing autonomy)?
   - Evidence from three weeks of interactions
   - Has the relationship changed?

---

### Week 6: Writing with AI Feedback

**Goal:** Produce essay with AI feedback; compare AI and self-assessment

**No new reading—writing week**

#### Day 1: Monday (3 hours)

**Writing:**
- Begin drafting independently
- Target: Introduction + first section
- Aim for ~600 words

**Essay Structure (1,500 words):**

**I. Introduction (200 words)**
- Frame the question: What's at stake in reducibility debate?
- Preview positions
- Your thesis

**II. Churchland's Reductive Neurophilosophy (400 words)**
- Her argument from Part II
- What would successful reduction look like?
- Her response to anti-reductionist arguments
- The "co-evolution" methodology
- Evidence/reasoning
- Strengths

**III. Critical Response (400 words)**
- Choose Merleau-Ponty OR Ingold as primary critic
- Their challenge to reductionism
- Why reduction fails or is inappropriate
- What does their approach capture that reduction loses?
- Strengths

**IV. Evaluation (400 words)**
- Your assessment: Who has stronger argument?
- What philosophical assumptions underlie disagreement?
- Are they addressing same phenomenon or different aspects?
- What does "reduction" mean in each framework?
- Is the debate resolvable or do they talk past each other?

**V. Conclusion (100 words)**
- Summarize your position
- Implications for understanding consciousness

**Tasks:**
- Draft independently first—write your own ideas before consulting AI
- Record time: writing vs. consulting

**Documentation:**
- Daily log: How much did you draft? How confident do you feel?
- Don't submit to Claude yet—wait until Day 2

---

#### Day 2: Tuesday (3 hours)

**Writing:**
- Continue independently
- Complete through Section III
- Aim for ~1,000 words total

**Mid-Draft Check-in:**
- **Submit to Claude:** "I've drafted Introduction and sections on Churchland and [Merleau-Ponty/Ingold] [paste]. Before continuing, I need feedback:
  1. Is my argument clear? Can you follow my reasoning?
  2. Am I representing thinkers accurately? Any misunderstandings?
  3. What needs clarification or strengthening?
  4. What am I missing or overlooking?"

- Receive feedback
- **Revise Sections I-III based on feedback**

**Documentation:**
- `/ai_interactions/week6_day2.md`
- Daily log: How did feedback change draft? Was it helpful? Did you agree with it?

---

#### Day 3: Wednesday (3 hours)

**Writing:**
- Complete Section IV (Evaluation) and Conclusion
- Full first draft (~1,500 words)

**Self-Check before submitting:**
- Have I answered the essay question?
- Is my argument clear?
- Have I fairly represented all positions?
- Is my evaluation substantive (not just "both have good points")?

**Full Draft Feedback:**
- **Submit to Claude:** "Here's my complete first draft [paste]. Please evaluate using the five-dimension rubric:
  1. Argument & Structure
  2. Knowledge & Understanding
  3. Integration of Perspectives
  4. Critical Engagement
  5. Style, Evidence & Presentation

For each dimension: What's working well? What needs improvement? What specific changes would strengthen the essay? Where am I weakest?"

- Receive detailed feedback
- Read carefully and consider

**Documentation:**
- `/ai_interactions/week6_day3.md`
- Daily log: How useful is feedback? Does it reveal blind spots? Do you agree with assessment?

---

#### Day 4: Thursday (3 hours)

**Revision:**
- Revise substantially based on feedback
- Focus on weakest dimensions identified by Claude
- This should be real revision, not just surface edits

**Targeted Help:**
- **If struggling with specific sections:**
  - "I'm working on critical evaluation [paste section]. You said it needs clearer connection to reduction question and deeper analysis of assumptions. Help me see what's missing—what should I be asking here?"

**Tasks:**
- Substantial revision: improve argument, evidence, analysis
- Deepen critical engagement
- Strengthen integration of perspectives

**Documentation:**
- `/ai_interactions/week6_day4.md`
- Daily log: Track specific improvements made

---

#### Day 5: Friday (2.5 hours)

**Final Revision:**
- Final improvements to argument and prose
- Proofread carefully
- Ensure citations are correct and complete

**Final Feedback:**
- **Submit to Claude:** "Here's my revised essay [paste]. Compared to first draft:
  1. How much has it improved?
  2. What remaining weaknesses do you see?
  3. What would move it to a higher grade band?
  4. Is it ready or does it need further work?"

- Final adjustments based on response

**Finalize essay**

**Documentation:**
- `/ai_interactions/week6_day5.md`
- Daily log

---

#### Day 6: Saturday (3 hours)

**AI ASSESSMENT REQUEST**

**Ask Claude:** "Please provide complete assessment of my final essay:

1. Evaluate using five-dimension rubric:
   - Argument & Structure
   - Knowledge & Understanding
   - Integration of Perspectives
   - Critical Engagement
   - Style, Evidence & Presentation

2. For each: Specific grade (First/2:1/2:2/Third/Fail) with detailed justification and evidence from essay

3. Overall indicative grade band with specific mark (e.g., 'Upper Second, 67')

4. Overall justification: Why this grade? What are the essay's main strengths and weaknesses?

5. What would move it to a higher band? Be specific."

**Receive and save AI's assessment**

---

**YOUR SELF-ASSESSMENT (800 words)**

Now assess independently, before comparing with AI:

**For each dimension:**
- What grade do you give?
- Why? (Strengths and weaknesses with specific evidence)
- How confident are you in this assessment?

**Overall grade:**
- First (70+), 2:1 (60-69), 2:2 (50-59), Third (40-49), Fail (<40)
- Specific mark if possible
- Overall justification

**Comparison with AI:**
- Where do assessments converge?
- Where do they diverge?
- If different, why? Who's right?
- How do you feel about AI's assessment—fair? Too harsh? Too generous?
- Is AI a reliable assessor? What are its limits?
- Would you trust AI assessment over your own? Why or why not?

**Documentation:**
- Save with essay
- `/ai_interactions/week6_day6.md`
- Daily log

---

#### Week 6 Final Ethnographic Reflection (Due Sunday)

**Length:** 1,000 words  
**Title:** "Learning With AI: Final Assessment"

**Culminating reflection:**

1. **How did AI shape learning over three weeks?**
   - Overall pattern
   - Evolution from Week 4 through Week 6

2. **What could I learn with AI that I couldn't alone?**
   - Specific capabilities AI enabled
   - Concrete evidence

3. **What did AI prevent or obscure?**
   - Where did it limit independent thinking?
   - Where did it create dependency?
   - What did I *not* develop because AI was available?

4. **How did relationship evolve?**
   - Week 4 vs. 5 vs. 6
   - More or less dependent over time?
   - More effective use?
   - Better questions?

5. **Essay feedback and assessment:**
   - Did AI feedback reveal genuine blind spots?
   - Or just confirm what I already knew?
   - Was AI's assessment accurate?
   - How does AI assessment compare to self-assessment?
   - Is AI good enough to replace human expertise in assessment?
   - What can't it evaluate?

6. **Where were AI's limits most apparent?**
   - What questions couldn't it answer well?
   - What feedback felt inadequate or generic?
   - Where did I most miss human interlocution?
   - What does human expertise provide that AI doesn't?

7. **Would I continue learning this way?**
   - Honest assessment—not what you "should" say
   - What would need to change?
   - What worked? What didn't?

---

**END OF PILOT B**

**Materials to Save:**
- Essay (final version)
- AI assessment (from Day 6)
- Self-assessment (800 words)
- All AI interactions (18 days worth)
- Three ethnographic reflections (Weeks 4, 5, 6)

**Wait for Week 7 before comparing with Pilot A**

---

## Week 7: Comparative Analysis

**Goal:** Systematically compare pilots and make evidence-based decision

### Days 1-2: Data Review and Pattern Recognition

#### Task 1: Review All Materials (6-8 hours)

Reread carefully:
- Both essays
- All daily logs from Pilot A (18 entries)
- All AI interactions from Pilot B (18 days)
- All reflections (6 total)
- Both self-assessments
- AI assessment (Pilot B)

---

#### Task 2: Create Comparison Tables

**Table 1: Learning Outcomes**

| Dimension | Pilot A (Autodidactic) | Pilot B (AI-Mediated) |
|-----------|------------------------|----------------------|
| Depth of understanding | [Rate 1-10] | [Rate 1-10] |
| Clarity of concepts | | |
| Confidence in knowledge | | |
| Quality of essay | | |
| Ability to explain topic | | |
| Accuracy of understanding | | |

Provide brief justification for each rating.

---

**Table 2: Learning Process**

| Aspect | Pilot A | Pilot B |
|--------|---------|---------|
| Total time invested | [hours] | [hours] |
| Time feeling confused | [%] | [%] |
| Time feeling productive | | |
| Times significantly stuck | | |
| How confusions resolved | [describe] | [describe] |
| Strategies developed | | |

---

**Table 3: AI Contribution (Pilot B)**

| AI Function | Value (1-10) | Evidence |
|-------------|--------------|----------|
| Clarifying concepts | | [Examples] |
| Identifying connections | | |
| Revealing blind spots | | |
| Structuring thinking | | |
| Essay feedback | | |
| Assessment accuracy | | |

---

#### Task 3: Return to Baseline

Reread Week 0 Pre-Pilot Self-Assessment.

**Write (500 words): "What I Now Understand That I Didn't Before"**

- How has understanding changed from baseline?
- Can you explain positions clearly now?
- What was your position in Week 0? What is it now?
- Substantial or marginal change?
- Which pilot contributed more to change?

---

### Days 3-5: Writing Comparative Analysis

**Assignment:** 2,500-word analysis

**Title:** "Learning With and Without the Machine: Pilot Evaluation"

#### Section 1: Quality of Understanding (800 words)

**Address:**

1. **Which condition produced deeper understanding?**
   - Evidence: Compare essays
   - Compare confidence ratings
   - Compare accuracy of representation

2. **Which allowed clearer connections?**
   - Between thinkers
   - Between positions
   - Between disciplines

3. **Where did understanding feel more secure?**
   - Where did you truly grasp material?
   - Where might you be wrong unknowingly?

4. **Can you explain positions clearly now?**
   - Nagel, Dennett, Churchland, Merleau-Ponty, Ingold
   - Could you defend essays to an expert?
   - Which thinkers do you understand best? Worst?

5. **Which essay is better? Why?**
   - Quality of argument
   - Accuracy of representation
   - Depth of analysis
   - Be honest—not diplomatic

**Required:**
- Specific examples from essays
- Quotes from logs/reflections showing understanding at different points
- Honest assessment—don't claim equality if false

---

#### Section 2: Learning Process (800 words)

**Address:**

1. **Which was more time-efficient?**
   - Hours spent
   - Productivity per hour
   - Time wasted vs. productive

2. **Where did you get stuck?**
   - Causes of confusion in each condition
   - How resolved (or not)?
   - Compare strategies and effectiveness

3. **Which felt more intellectually satisfying?**
   - Where did genuine insight occur?
   - "Aha" moments in each condition

4. **Which was more frustrating? Why?**
   - Different kinds of frustration?

5. **How did reading differ?**
   - Pace, comprehension, engagement
   - With vs. without AI framing
   - Did AI help or interfere with direct engagement?

6. **How did writing differ?**
   - Planning, drafting, revising
   - With vs. without feedback
   - Did AI improve essay or just confirm your work?

7. **Which developed independent thinking?**
   - Which taught you to think for yourself?
   - Which risked dependency or intellectual passivity?

**Required:**
- Time accounting from daily logs
- Specific difficulties and how they were (or weren't) resolved
- Deep reflection on learning process, not surface description

---

#### Section 3: The AI Contribution (600 words)

**Address:**

1. **What did AI do well?**
   - Specific valuable contributions
   - What kinds of questions/tasks did it handle effectively?

2. **What did AI do poorly?**
   - Inadequate or misleading guidance
   - What kinds of questions/tasks did it struggle with?

3. **When was AI most valuable?**
   - Critical difference-making moments
   - What would have been impossible without it?

4. **When was AI unhelpful?**
   - Moments you ignored it or it misled
   - Generic responses that added nothing

5. **Did AI reveal genuine blind spots?**
   - Specific improvements it enabled
   - Or did it just confirm what you already knew?

6. **Was AI assessment accurate?**
   - Compare with self-assessment
   - Is AI reliable assessor?
   - What are its assessment limits?

7. **Could you achieve same learning more simply?**
   - Without AI apparatus?
   - What did AI actually add to learning?
   - Was the complexity justified?

**Required:**
- Critical evaluation—not uncritically positive or negative
- Evidence from interactions throughout three weeks
- Honest assessment of whether AI justifies complexity

**Sub-section: The Mumford Question (within 600 words)**

*"The AI as Expert System: What Kind of Authority?"*

Drawing on Mumford's "authoritarian" vs. "democratic" technics:

- What kind of knowledge does AI provide? (Propositional, systematic, expert?)
- What doesn't it provide? (Embodied, tacit, experiential?)
- Does AI feel like "megamachine"—efficient but flattening?
- Or genuine partnership enhancing autonomy?
- Does AI create expert-dependency (as Smail critiques)?
- Or develop independent judgment?
- How did this relationship evolve over three weeks?

**Evidence:** Patterns from all three weeks of Pilot B

---

#### Section 4: Decision and Justification (300 words)

**Choose ONE option and justify with evidence:**

### Option A: Proceed with Full Module 1

**State:** "I will proceed with the full four-year programme using AI framework."

**Justify:**
- Why AI adds value justifying complexity
- Specific evidence of improved outcomes
- What you learned about using AI effectively
- Modifications needed based on pilot findings (if any)

**Next steps:** Timeline for Module 1 start

---

### Option B: Abandon AI, Study Autodidactically

**State:** "I will not proceed with AI-mediated learning. I will study autodidactically."

**Justify:**
- Why AI added little value
- Evidence Pilot A was equally or more effective
- What AI prevented or obscured
- Alternative plan for continued study

**Next steps:** How you'll structure future study without AI

---

### Option C: Redesign Framework

**State:** "I will modify the framework before proceeding."

**Justify:**
- Mixed results suggest needed changes
- What worked and should be retained
- What didn't and needs modification
- Specific proposed changes to framework

**Next steps:** Redesign process and timeline for modified pilot or Module 1

---

### Option D: Extend Pilot

**State:** "I need more data. I will extend the pilot."

**Justify:**
- Why current data insufficient for decision
- What additional comparison would clarify
- Specific extended pilot design

**Next steps:** Design of extension (topics, duration, methods)

---

**Whichever you choose:**
- Defend with evidence from pilot data
- Be honest about what data shows
- Don't let sunk costs (time designing framework) influence decision
- Serve your learning, not the project's existence
- Negative results are legitimate and valuable findings

---

### Day 6: Submission and Final Reflection

#### Task 1: Submit Analysis to Claude

**Request:**
"I've completed my comparative analysis [paste]. Please provide:
1. Detailed feedback on the analysis itself
2. Assessment of whether my decision is well-justified by evidence
3. Observations about learning patterns I may have missed
4. Suggestions for proceeding (regardless of which option I chose)
5. Critical questions: Where is analysis strongest? Weakest? What am I not seeing? What am I avoiding?"

Consider feedback carefully.

---

#### Task 2: Final Pilot Reflection (500 words)

**Title:** "What This Pilot Taught Me"

Beyond decision about AI framework, what did you learn about:

1. **Your own learning style and capabilities?**
   - Strengths and weaknesses as learner
   - What you need to learn effectively

2. **The nature of consciousness and mind-body problem?**
   - Genuine philosophical insights
   - How has your position evolved?

3. **The challenge of self-directed study?**
   - What makes it work or fail?

4. **What you need to learn effectively?**
   - What resources, structures, guidance?

5. **The relationship between reading, thinking, writing?**
   - How do they interact in learning?

6. **The value and limits of AI as learning partner?**
   - What AI can and cannot provide
   - Its role in intellectual development

---

#### Task 3: If Proceeding to Module 1

- Take 3-4 days off
- Return to framework documents
- Make modifications based on pilot findings
- Set Module 1 start date
- Acquire Module 1 readings
- Prepare first week materials

**Begin with documented awareness of pilot findings**

---

#### Task 4: If Not Proceeding

**Write addendum (300 words):**

**Title:** "Why This Framework Wasn't Right for Me"

- What alternative approach will you take?
- What was valuable from pilot that you'll retain?
- No shame in this outcome—pilot did its job by revealing this

---

## Complete Reading List

### Pilot A: Consciousness and Intentionality

**1. Bermúdez, José Luis (2020)**
*Philosophy of Psychology: A Contemporary Introduction* (2nd ed.)
Routledge
- Chapter 5: Consciousness and Subjectivity (~40 pages)

**2. Nagel, Thomas (1974)**
"What Is It Like to Be a Bat?"
*The Philosophical Review*, 83(4), 435-450
Also in: Nagel, T. (1979). *Mortal Questions*. Cambridge University Press.

**3. Dennett, Daniel C. (1991)**
*Consciousness Explained*
Little, Brown and Company
- Chapter 2: Explaining Consciousness (~30 pages)

---

### Pilot B: Mind-Body Problem and Embodiment

**1. Bermúdez, José Luis (2020)**
*Philosophy of Psychology: A Contemporary Introduction* (2nd ed.)
Routledge
- Chapter 8: The Mind-Body Problem (~35 pages)

**2. Merleau-Ponty, Maurice (1945/2012)**
*Phenomenology of Perception* (D. Landes, Trans.)
Routledge
- Preface and Introduction (~30 pages)

**3. Churchland, Patricia Smith (1986)**
*Neurophilosophy: Toward a Unified Science of the Mind-Brain*
MIT Press
- **Part II: "Recent Developments in the Philosophy of Science" (~100-150 pages)**

**Note:** The book is organized in three parts. Part I ("Some Elementary Neuroscience") provides neuroscience background—skip unless you need context. Part III ("A Neurophilosophical Perspective") covers dated 1980s computational models—skip. Part II contains her philosophical arguments for reductive neurophilosophy, her account of intertheoretic reduction, responses to dualist objections, and the "co-evolution" methodology. This is what you need for understanding her position in philosophy of mind debates.

**4. Ingold, Tim (2000)**
*The Perception of the Environment: Essays on Livelihood, Dwelling and Skill*
Routledge
- Chapter 1: The Organism and its Environment (pp. 13-25)
- Chapter 16: On Weaving a Basket (pp. 339-348)

**5. Mumford, Lewis (1964)**
"Authoritarian and Democratic Technics"
*Technology and Culture*, 5(1), 1-8

---

### Optional Readings

**Chalmers, David J. (1995)**
"Facing Up to the Problem of Consciousness"
*Journal of Consciousness Studies*, 2(3), 200-219

**Stanford Encyclopedia of Philosophy**
- Entry on "Consciousness"
- Entry on "Dualism"
- Entry on "Physicalism"
- Entry on "The Mind-Body Problem"

---

## Templates and Forms

### Daily Log Template (Pilot A)

```markdown
# Daily Log: Pilot A (Autodidactic)

## Date: [Date]

| Field         | Value |
| ------------- | ----- |
| Start Time    |       |
| End Time      |       |
| Activity      |       |
| Energy (1-5)  |       |
| Mood (1-5)    |       |
| Clarity (1-5) |       |
| Quality (1-5) |       |

### What I Read/Studied:

### What Was Difficult:

### Questions That Arose:

### How I Addressed Confusion (or didn't):

### What I Wish I Had Access To:

### Confidence Level in Today's Learning (1-10):

### Additional Notes:
```

---

### Weekly Reflection Template (Pilot A)

```markdown
# Weekly Reflection: Pilot A, Week [X]

## Date: [Date]

### What I Understood Well This Week:
- 

### What Remains Confusing:
- 

### How I Handled Difficulties:
- 

### Strategies I Developed:
- 

### What I Wish I Had Access To:
- 

### Confidence Assessment (1-10):
[Rating]

### Why This Rating:
```

---

### Ethnographic Reflection Template (Pilot B)

```markdown
# Ethnographic Reflection: Week [X], Pilot B

## Date: [Date]

### How did AI frame authority and expertise this week?

### What questions did I ask AI, and how adequate were responses?

### Where did I disagree with AI feedback, and why?

### What did AI reveal that I had missed?

### What did I wish for that AI couldn't provide?

### How did the AI relationship feel this week?
(collaborative, directive, frustrating, etc.)

### Evidence of dependency vs. autonomy:

### Additional observations:
```

---

### Essay Self-Assessment Template

```markdown
# Essay Self-Assessment

## Essay Title: [Title]
## Date: [Date]
## Pilot: [A or B]

---

## Five-Dimension Assessment

### 1. Argument & Structure
**Grade:** [First/2:1/2:2/Third/Fail]

**Justification:**
- Strengths:
- Weaknesses:
- Specific evidence from essay:

---

### 2. Knowledge & Understanding
**Grade:** [First/2:1/2:2/Third/Fail]

**Justification:**
- Strengths:
- Weaknesses:
- Specific evidence:

---

### 3. Integration of Perspectives
**Grade:** [First/2:1/2:2/Third/Fail]

**Justification:**
- Strengths:
- Weaknesses:
- Specific evidence:

---

### 4. Critical Engagement
**Grade:** [First/2:1/2:2/Third/Fail]

**Justification:**
- Strengths:
- Weaknesses:
- Specific evidence:

---

### 5. Style, Evidence & Presentation
**Grade:** [First/2:1/2:2/Third/Fail]

**Justification:**
- Strengths:
- Weaknesses:
- Specific evidence:

---

## Overall Assessment

**Overall Indicative Grade:** [First (70+) / 2:1 (60-69) / 2:2 (50-59) / Third (40-49) / Fail (<40)]

**Specific Mark (if applicable):** [e.g., 67]

**Overall Justification:**

**What would need to improve to move to higher band:**
```

---

## Assessment Rubric

### Five-Dimension Assessment Framework

Use this rubric for self-assessment and to understand AI assessment criteria.

---

### 1. Argument & Structure

**First-Class (70-100):**
- Crystal-clear thesis, maintained throughout
- Sophisticated, original argument structure
- Logical progression of ideas
- Elegant transitions
- Conclusion synthesizes rather than summarizes

**Upper Second (60-69):**
- Clear thesis and argument
- Logical structure
- Good transitions
- Some originality
- Solid conclusion

**Lower Second (50-59):**
- Thesis present but may drift
- Basic logical structure
- Some organizational issues
- Adequate transitions
- Conclusion adequate

**Third (40-49):**
- Weak or unclear thesis
- Structural problems
- Poor organization
- Weak transitions
- Weak conclusion

**Fail (<40):**
- No clear thesis
- Incoherent structure
- Disorganized
- No real argument

---

### 2. Knowledge & Understanding

**First-Class (70-100):**
- Deep, nuanced understanding of all positions
- Accurate representation of complex arguments
- Sophisticated grasp of concepts
- Engages with subtle distinctions
- No significant errors

**Upper Second (60-69):**
- Good understanding of main positions
- Accurate representation
- Solid grasp of key concepts
- Some engagement with complexity
- Minor errors only

**Lower Second (50-59):**
- Adequate understanding
- Mostly accurate representation
- Basic grasp of concepts
- Limited engagement with complexity
- Some errors

**Third (40-49):**
- Superficial understanding
- Some misrepresentations
- Weak grasp of concepts
- Significant errors

**Fail (<40):**
- Fundamental misunderstanding
- Major misrepresentations
- Serious conceptual confusion

---

### 3. Integration of Perspectives

**First-Class (70-100):**
- Sophisticated synthesis of different views
- Identifies deep connections and tensions
- Brings perspectives into genuine dialogue
- Original insights from integration

**Upper Second (60-69):**
- Good integration of perspectives
- Identifies key connections
- Meaningful comparison
- Some synthesis

**Lower Second (50-59):**
- Adequate comparison
- Some connections identified
- Limited integration
- Mostly separate treatment

**Third (40-49):**
- Weak comparison
- Few connections
- Largely separate treatment

**Fail (<40):**
- No integration
- Perspectives not compared

---

### 4. Critical Engagement

**First-Class (70-100):**
- Sophisticated critical analysis
- Identifies unstated assumptions
- Evaluates strengths and weaknesses fairly
- Original critical insights
- Goes beyond source material

**Upper Second (60-69):**
- Good critical analysis
- Identifies key assumptions
- Fair evaluation
- Some original insight
- Engages critically with sources

**Lower Second (50-59):**
- Adequate critical analysis
- Some evaluation
- Mostly descriptive
- Limited originality

**Third (40-49):**
- Weak critical analysis
- Largely descriptive
- Little evaluation
- No originality

**Fail (<40):**
- No critical analysis
- Purely descriptive
- Uncritical acceptance

---

### 5. Style, Evidence & Presentation

**First-Class (70-100):**
- Excellent prose: clear, precise, elegant
- Strong evidence for all claims
- Proper citations throughout
- No errors
- Professional presentation

**Upper Second (60-69):**
- Clear, effective prose
- Good evidence
- Proper citations
- Few minor errors
- Good presentation

**Lower Second (50-59):**
- Adequate prose
- Some evidence
- Citations present but may have issues
- Some errors
- Adequate presentation

**Third (40-49):**
- Weak prose
- Limited evidence
- Citation problems
- Multiple errors
- Poor presentation

**Fail (<40):**
- Very poor prose
- Little/no evidence
- Major citation problems
- Numerous errors

---

## Final Notes

### On Honesty

The pilot only works if you're brutally honest. If Pilot A produced equal or better understanding than Pilot B, that's a valid and important finding. Don't proceed with an elaborate AI framework just because you've invested effort in designing it.

### On Flexibility

If during either pilot you realize the comparison isn't working as designed (readings too hard/easy, time estimates off, structure problematic), pause and adjust. Document why. This is more honest than forcing through flawed design.

### On "Negative" Results

If pilot reveals AI adds little value, that's not failure—it's success. You've discovered this before committing to 16 weeks of Module 1. The pilot has done its job.

### On Beginning

Set your start date.  
Acquire your readings.  
Create your folders.  
Write your baseline self-assessment.

Then begin.

---

**END OF PILOT CURRICULUM**

---

*This curriculum prepared November 2025 for experimental pilot in AI-mediated learning. All intellectual work remains the student's own; AI provides guidance and feedback only.*

*Version 1.1: Corrected Churchland reading from "Chapter 1 (~30 pages)" to "Part II: Recent Developments in the Philosophy of Science (~100-150 pages)" with adjusted Week 5 schedule.*
